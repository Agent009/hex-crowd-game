...
- **2026-06-11 top-level boundary:** Added a reusable `src/components/ErrorBoundary.tsx` wrapping the entire app (`Game`), so a render/runtime error in any UI surface (lobby, HUD, hero panel) shows an accessible recovery screen with Try-again/Reload instead of a blank page.
